...
import { updateRoomAction } from '@/lib/action';
...
const EditForm = ({ amenities, room }: EditFormProps) => {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [image, setImage] = useState(room.image)
  const [isTransition, startTransition] = useTransition()
  const [selectedAmenities, setSelectedAmenities] = useState<string[]>(room.roomAmenities.map((ra) => ra.amenityId))
  const [uploadError, setUploadError] = useState<string | null>(null)
  const [state, formAction, isPending] = useActionState(updateRoomAction.bind(null, image, room.id), null)

  const removeImage = () => {
    const currentImage = image
    const hasNewUpload = currentImage !== "" && currentImage !== room.image
    setImage("")
    setUploadError(null)
    ...
    <form action={formAction} className="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          name="description"
          rows={4}
          defaultValue={state?.values?.description ?? room.description}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            name="price"
            min={0}
            defaultValue={state?.values?.price ?? room.price}
            placeholder="800000"
            className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors duration-200 text-sm"
          />
          {state?.error?.price && (
        ...
        <div>
          ...
          <input
            ...
            name="capacity"
            min={1}
            defaultValue={state?.values?.capacity ?? room.capacity}
            placeholder="2"
            className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors duration-200 text-sm"
          />
      ...
      <div className="flex items-center gap-4 pt-4">
        ...
        >
          ...
          {isPending || isTransition ? 'Mengupload...' : 'Ubah Data'}
        </button>
        <Link
